...
import { CustomblockDto, TeamDto } from "apis/dto"
...
//     apiCaller.blockApi.getList(),
//   ])

//   if (apiUtil.isErrorResponse(teamListRes)) {
//     return {
//       props: { teamList: [], customblockList: [], errorMsg: teamListRes.message },
//     }
//   } else if (apiUtil.isErrorResponse(customblockListRes)) {
//     return {
//       props: { teamList: [], customblockList: [], errorMsg: customblockListRes.message },
//     }
//   }
...
const CustomBlocksPage = () => {
  ...
  useAsyncEffect(async () => {
    ...
    const blockListRes = await apiManager.blockApi.getList()
    if (apiUtil.isErrorResponse(teamListRes)) {
        ...
        <Col className={styles.contents_container}>
          ...
          <Row className={styles.block_list}>
            ...
            {searchBlockListState.map((block, index) => {
              return (
                <Col className={styles.block_item} key={index} onClick={() => setIsSelectBlockId(block.customblockID)}>
          ...
          <Row className={styles.reference_list}>
            {[1, 2, 3, 4, 5, 6, 7, 8].map((project, index) => (
              <Col className={styles.block_item} key={index} onClick={() => setSelectBlock(project)}>
                <Row className={styles.block_item_img} justify={"center"} align={"middle"}>
                  img
                </Row>
                <Row className={styles.block_item_name} justify={"center"} align={"middle"}>
            ...
            {isSelectBlockId !== null ? (
              <Space className={styles.block_detail_wrapper} direction="vertical" size={0}>
                ...
                {/* <Space direction="vertical" className={styles.project_function}>
                  ...
                  <Col className={styles.block_properties_desc}>{selectBlock.type}</Col>
                  <div className={styles.divider} />
                </Space> */}
                ...
                <Space direction="vertical" className={styles.project_spec} size={10}>
