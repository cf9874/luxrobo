import { NextPage } from "next"
import { apiManager, apiUtil } from "@/apis"
import { Divider, Image } from "antd"
import styles from "./companyDetail.module.scss"
import { PageFrameFooter } from "@/components/layout"
import { CompanyDescDetail, CompanyDesc, CompanyDescBType } from "@/components/pages/company/companyDesc"
import { COMPANYDETAIL, ERROR_MSG } from "@/const"
import { useRouter } from "next/router"
import { useAsyncEffect } from "@/hooks"
import { useState } from "react"
import { BasicInputArea } from "@/components/Input"
import { FileButton } from "@/components/Button"
import { CompanyDto } from "@/apis/dto"
import { API_ERROR_MSG } from "@/const/api.error.const"
import { ObjectDescription } from "@/components/Description"
import { JSONFormChecker } from "@/utils"
import { CheckOutlined, CloseOutlined } from "@ant-design/icons"

//------------------------------------------------------------------------------------------------------
// [Main]
const OrderDetail: NextPage = () => {
  const router = useRouter()
  //-------------------------------------------------------------------------
  // [Datas]
  const [companyID, setCompanyID] = useState<number>(undefined)

  // (Description A)
  const [companyDataA, setCompanyDataA] = useState<CompanyDto>(undefined)

  // (Description B)
  const [companyDataB, setCompanyDataB] = useState<CompanyDescBType>(undefined)

  const [thumbNail, setThumbNail] = useState<File>(null)
  const [thumbNailURL, setThumbNailURL] = useState<string>(null)

  //-------------------------------------------------------------------------
  // 초기화
  useAsyncEffect(async () => {
    if (!router.query.id) return

    // setContactID
    setCompanyID(Number(router.query.id))

    const companyResponse = await apiManager.companyApi.getDetail(Number(router.query.id))

    if (apiUtil.isErrorResponse(companyResponse)) {
      if (apiUtil.signChecker(companyResponse)) {
        alert(API_ERROR_MSG.NEEDLOGIN)
        apiUtil.removeUserData()
        void router.push("/signin")
        return
      }

      alert(companyResponse.message)
      return
    }

    // setDescA
    setCompanyDataA(companyResponse)

    // setDescB
    const { companyID, image, ...companyRawData } = companyResponse

    setCompanyDataB({
      rawData: JSON.stringify(companyRawData, null, 4),
    })
  }, [router.query.id])

  useAsyncEffect(async () => {
    if (!companyDataB) return
    if (!JSONFormChecker(companyDataB.rawData)) return

    // JSON 형태 변환 가능 시 -> DescA(CompanyDto) 초기화
    setCompanyDataA({
      companyID: companyDataA.companyID,
      image: companyDataA.image,
      ...JSON.parse(companyDataB.rawData),
    })
  }, [companyDataB])

  //-------------------------------------------------------------------------
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files[0]

    if (file) {
      setThumbNail(file)
      setThumbNailURL(URL.createObjectURL(file))
    }
  }

  // [footer Handler]
  const saveHandler = () => {
    console.log("Save: Order")
    console.log("companyID", companyID)
    if (!JSONFormChecker(companyDataB.rawData)) {
      alert(ERROR_MSG.WRONGINPUT)
      return
    }

    const companyEdit = {
      name: companyDataA.name,
      number: companyDataA.number,
      webpage: companyDataA.webpage,
      ["e-mail"]: companyDataA["e-mail"],
      price: JSON.stringify(companyDataA.price),
      date: JSON.stringify(companyDataA.date),
      company_image: thumbNail,
    }
    console.log("companyEdit", companyEdit)

    const res = apiManager.companyApi.editCompany(companyID, companyEdit)
    if (apiUtil.isErrorResponse(res)) {
      if (apiUtil.signChecker(res)) {
        alert(API_ERROR_MSG.NEEDLOGIN)
        apiUtil.removeUserData()
        void router.push("/signin")
        return
      }

      alert(res.message)
      return
    }

    router.push("/company")
  }

  const cancelHandler = () => {
    console.log("Cancel: Order")
    router.push("/company")
  }

  //-------------------------------------------------------------------------
  // [Content Render]
  const CompanyRender = ({ key, pm_value }) => {
    switch (key) {
      // desc A
      case "image":
        return (
          <div className={styles.descA_image_wrapper}>
            {thumbNail ? (
              <Image src={thumbNailURL} alt={"ThumbNail"} preview={false} className={styles.descA_image_image}></Image>
            ) : (
              <Image src={pm_value} alt={"ThumbNail"} preview={false} className={styles.descA_image_image}></Image>
            )}
            <FileButton option={"image"} handleFile={handleFile} className={styles.descA_image_button}>
              이미지 교체
            </FileButton>
          </div>
        )
      case "webpage":
        return <a href={pm_value}>{pm_value}</a>
      case "eMail":
        return <a href={`mailto:${pm_value}`}>{pm_value}</a>
      // desc B
      case "rawData":
        return (
          <div className={styles.descB_rawData_wrapper}>
            <BasicInputArea
              option={"big"}
              value={pm_value}
              onChange={e => setCompanyDataB({ ...companyDataB, [key]: e.target.value })}
            ></BasicInputArea>
            {JSONFormChecker(pm_value) ? (
              <div className={styles.descB_rawData_formCheck_wrapper}>
                <CheckOutlined className={`${styles.descB_rawData_formCheck} ${styles.descB_rawData_formCheck_true}`} />
              </div>
            ) : (
              <div className={styles.descB_rawData_formCheck_wrapper}>
                파일 형식이 맞지 않습니다.{" "}
                <CloseOutlined
                  className={`${styles.descB_rawData_formCheck} ${styles.descB_rawData_formCheck_false}`}
                />
              </div>
            )}
          </div>
        )
      default:
        if (typeof pm_value === "object" && !Array.isArray(pm_value) && pm_value !== null)
          return <div>{JSON.stringify(pm_value)}</div>
        return <div>{pm_value}</div>
    }
  }

  //-------------------------------------------------------------------------
  // [UI 출력]
  return (
    <PageFrameFooter
      titleKey={COMPANYDETAIL}
      handler={{
        saveHandler: saveHandler,
        cancelHandler: cancelHandler,
      }}
    >
      <div className={styles.container}>
        <ObjectDescription
          descInfo={CompanyDesc.descA}
          descDetails={CompanyDescDetail}
          data={companyDataA}
          render={CompanyRender}
        ></ObjectDescription>
        <Divider></Divider>
        <ObjectDescription
          descInfo={CompanyDesc.descB}
          descDetails={CompanyDescDetail}
          data={companyDataB}
          render={CompanyRender}
        ></ObjectDescription>
        <Divider></Divider>
      </div>
    </PageFrameFooter>
  )
}

export default OrderDetail
