...
import { ShippingIndex, ShippingIndexDetail, ShippingIndexType } from "@/components/pages/shipping/indexDesc"
import { Divider, Pagination } from "antd"
import { useState } from "react"
import { useAsyncEffect, useChanges, useInputs } from "@/hooks"
import { SHIP, paginationSize, shippingCondition } from "@/const"
...
//------------------------------------------------------------------------------------------------------
// [Main]
export const ShipPage: NextPage = () => {
  ...
  const [CL_tableData, setCLTableData] = useState<ShippingIndexType[]>([] as ShippingIndexType[])
  ...
  // (Excel)
  const excelHeader = ShippingIndex.keys
    .filter(key => key !== "manage")
    .map(key => {
      return {
        label: ShippingIndexDetail[key].label,
        key: key,
      }
    })
  ...
  useAsyncEffect(async () => {
    const shippingResponse = await apiManager.orderApi.searchShippingList("", { is_match: true, range: 1 })
    ...
    const modifiedShippingDatas = shippingResponse.map((item, index) => {
      return {
        index: index + 1,
        key: item.seq_no,
        manage: item.seq_no,
    ...
    setCLTableData(modifiedShippingDatas)
  }, [])
      ...
      setExcelData(
        CL_tableData.map(item => {
          ...
          ShippingIndex.keys
            ...
            .forEach(key => {
              if (key === "order_step") {
                excelItem[key as string] = shippingCondition[item[key]]
  ...
  const searchHandler = async () => {
    ...
    const res = await apiManager.orderApi.searchShippingList(searchKeywordInput.value, {
  ...
  const resetHandler = async () => {
    ...
    const res = await apiManager.orderApi.searchShippingList("", {
      is_match: true,
      range: 1,
    })
    if (apiUtil.isErrorResponse(res)) {
  ...
  //------------------------------------------------------------------------------------------------------
  // [Render]
  const ShippingIndexRender = ({ index, key, pm_value }) => {
    switch (key) {
      case "order_step":
        return <div>{shippingCondition[pm_value]}</div>
      case "manage":
        return (
          <BasicButton
            onClick={() => {
    ...
    <PageFrame titleKey={SHIP}>
      <div className={styles.container}>
        ...
        <div className={styles.tableTopButton}>
          <ExcelButton headers={excelHeader} value={excelData} filename="ShippingList" />
        </div>
        <BasicTable
          option={"indexTable"}
          tableInfo={ShippingIndex}
          tableDetails={ShippingIndexDetail}
          render={ShippingIndexRender}
